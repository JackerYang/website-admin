...
export default {
  ...
  methods: {
    ...
    // 获取数据
    getData(params) {
      getOrgImpressionList(params).then(res => {
        if (res.data.res) {
      ...
      this.$confirm({
        ...
        onOk: () => {
          return new Promise(resolve => {
            delOrgImpression(selectedRowKeys).then(res => {
              if (res.data.res) {
      ...
      if (cancelLen) {
        this.$confirm({
          ...
          content: (
            <div>
              <p>
                当前选中了{len}条企业印象，其中未发布数量为{cancelLen}
                条，确定发布这{cancelLen}条企业印象？
              </p>
              <p style="color: #999;">已发布的企业印象不受影响</p>
            </div>
          ),
          onOk: () => {
            return new Promise(resolve => {
              orgImpressionPublish(ids).then(res => {
                if (res.data.res) {
                  this.$Message.success(res.data.msg);
                  // 刷新列表
                  this.getData(this.filterCondition);
        ...
        this.$info({
          ...
          content: (
            <div>
              <p>
                当前选中了{len}
                条企业印象，其中未发布数量为0条，无需执行发布操作！
              </p>
              <p style="color: #999;">无可发布的企业印象</p>
            </div>
          )
        });
      }
      ...
      if (publishLen) {
        this.$confirm({
          ...
          content: (
            <div>
              <p>
                当前选中了{len}条企业印象，其中已发布数量为{publishLen}
                条，确定撤销这{publishLen}条企业印象？
              </p>
              <p style="color: #999;">未发布的企业印象不受影响</p>
            </div>
          ),
          onOk: () => {
            return new Promise(resolve => {
              orgImpressionCancel(ids).then(res => {
                if (res.data.res) {
                  this.$Message.success(res.data.msg);
                  // 刷新列表
                  this.getData(this.filterCondition);
        ...
        this.$info({
          ...
          content: (
            <div>
              <p>
                当前选中了{len}
                条企业印象，其中已发布数量为0条，无需执行撤销操作！
              </p>
              <p style="color: #999;">无可撤销的企业印象</p>
            </div>
          )
        });
      }
