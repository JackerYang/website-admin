<template>
  <div class="company-impression-edit">
    <fn-template
      :isPreview="true"
      :title="isAdd ? '企业印象 - 添加' : '企业印象 - 编辑'"
      @on-save="save"
      @on-cancel="cancel"
...
import { editOrgImpression, getOrgImpression } from "@/api/api";
...
export default {
  ...
  methods: {
    save() {
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          editOrgImpression(obj).then(res => {
            if (res.data.res) {
    ...
    ready(e) {
      this.editor = e;
      if (!this.isAdd) {
        getOrgImpression(this.$route.path.split("/").reverse()[0]).then(res => {
          if (res.data.res) {
            let {
              title,
              subTitle,
